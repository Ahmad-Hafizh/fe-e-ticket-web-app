...
const EventDetailPage: React.FC<IEventDetailPage> = ({ params }) => {
  ...
  let isCustomer;
  if (eventData) {
    const filteredCustomers = eventData.customer.filter((cust: any) => cust.email === user.email);
  ...
  const submitTransactionDetails = () => {
    ...
    const filteredPayloadDetails = payloadDetails.filter((value) => value !== null);
    ...
    <>
      <div className="bg-white w-full h-full lg:px-48 py-10 flex flex-col gap-20 ">
        <div className="bg-white lg:grid lg:grid-cols-[2fr_1fr] relative ">
          <div className="lg:flex lg:flex-col lg:items-end lg:justify-end">
            <div className="bg-white w-full h-full px-10 md:px-20 lg:px-6 pt-10 lg:pb-4 pb-0 flex flex-col gap-5">
              {loading ? <img src={eventData.imgEvent} className="w-full rounded-lg shadow-sm lg:mb-3" /> : <Skeleton className="w-full rounded-lg shadow-sm lg:mb-3" />}
              <img src={eventData.imgEvent} className="w-full rounded-lg shadow-sm lg:mb-3" />
              <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold md:text-3xl lg:text-4xl">{eventData.title}</h1>
            ...
            <div className="bg-white w-full h-full px-10 md:px-20 lg:px-6 pb-6 pt-2 flex flex-col gap-1">
              {eventData.endDate ? (
                <h3 className="text-sm md:text-lg w-full flex items-center gap-1">
                  <MdDateRange /> <span className="font-bold hidden md:inline">Date :</span> {eventData.startDate.slice(0, 10)} - {eventData.endDate.slice(0, 10)}
                </h3>
              ) : (
                <h3 className="text-sm md:text-lg w-full flex items-center gap-1">
                  <MdDateRange /> <span className="font-bold hidden md:inline">Time :</span>
                  {eventData.startDate} ({eventData.timezone})
                </h3>
              )}
              <h3 className="text-sm md:text-lg w-full flex items-center gap-1">
                <IoMdTime /> <span className="font-bold hidden md:inline">Time :</span> {eventData.startTime} - {eventData.endTime} ({eventData.timezone})
              </h3>

              <h3 className="text-sm md:text-lg w-full flex items-center gap-1">
                <IoLocationOutline />
                <span className="font-bold hidden md:inline"> Location :</span> {eventData.event_location.address_name}, {eventData.event_location.address}, {eventData.event_location.zipcode}
              ...
              <div className="bg-white w-full h-full px-10 md:px-20 lg:px-6 py-3 flex flex-col lg:hidden">
                ...
                <div className="ticket-container">
                  {eventData.ticket_types.map((ticket: any, index: number) => {
                    return (
                      <div key={index} className="ticket flex justify-between py-2 lg:py-4 lg:px-10 my-3 items-center bg-gray-100 rounded-lg shadow-md px-3">
                        <div>
                          <h1 className="text-lg font-bold">{ticket.types}</h1>
                          {ticket.price === 0 ? <h2 className="text-md">Price: FREE</h2> : <h2 className="text-md">Price: IDR.{ticket.price}</h2>}
                          {ticket.quantity_available === 0 ? <></> : <h2 className="text-sm">Only {ticket.quantity_available} left.</h2>}
                        </div>
                        <div className="flex gap-2 items-center">
                          {ticket.quantity_available === 0 ? (
                            ...
                            <>
                              ...
                              <select className="p-2" onChange={(e) => updateTicketQuantity(index, parseInt(e.target.value))} value={ticketQuantities[index]}>
                    ...
                    {localStorage.getItem('tkn') || sessionStorage.getItem('tkn') ? (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          const payloadEventAndTicket = basicGetApi.post('/transaction/details', payload);
                          sessionStorage.setItem('transaction-data', JSON.stringify(payloadUltimate));
            ...
            <div className="bg-white w-full h-full px-10 md:px-20 lg:px-6 py-5 flex flex-col">
              ...
              <div className="ticket flex gap-2 py-2 my-3 items-center justify-start bg-gray-50 rounded-lg shadow-md px-3">
                <img src={eventData.organizer.organizer_logo} className="w-10 h-10 rounded-full" />
                <div>
                  <h1 className="text-md font-bold">{eventData.organizer.organizer_name}</h1>
                  <h1 className="text-sm">{eventData.organizer.organizer_email}</h1>
                </div>
              </div>
            </div>
            {isCustomer && !isReviewed ? (
              <div className="bg-white w-full h-full px-10 md:px-20 lg:px-6 py-3 flex flex-col gap-6">
                <h1 className="text-xl font-bold">What do you think of this event?</h1>
            ...
            {eventData.review[0] && (
              <div className="bg-white w-full h-full px-10 md:px-20 lg:px-6 py-5 flex flex-col">
                <h1 className="text-xl font-bold my-3">What they say about this event..</h1>
                <div className="ticket flex flex-col justify-between items-center gap-6">
                  {eventData.review.map((value: any, index: number) => {
                    return (
                      <div className="bg-gray-200 rounded-lg shadow-md px-5 pt-4 w-full flex flex-col gap-3" key={index}>
                        <div className="flex justify-between items-center">
                          <div className="avatar flex items-center gap-3">
                            <img src={value.user.pfp_url} className="rounded-full h-10 w-10" />
                            <h1 className="text-lg font-bold">{value.user.name}</h1>
                          </div>
                          <div className="score">
              ...
              {isFinished ? (
                ...
                <div className="bg-white w-full h-full px-10 md:px-32 lg:px-4 py-10 flex flex-col sticky top-0">
                  ...
                  <div className="ticket-container">
                    {eventData.ticket_types.map((ticket: any, index: number) => {
                      return (
                        <div key={index} className="ticket flex justify-between py-2 lg:py-4 lg:px-8 my-3 items-center bg-gray-100 rounded-lg shadow-md px-3">
                          <div>
                            <h1 className="text-lg font-bold">{ticket.types}</h1>
                            {ticket.price === 0 ? <h2 className="text-md">Price: FREE</h2> : <h2 className="text-md">Price: IDR.{ticket.price}</h2>}
                            {ticket.quantity_available === 0 ? <></> : <h2 className="text-sm">Only {ticket.quantity_available} left.</h2>}
                          ...
                          <div className="flex gap-2 items-center">
                            {ticket.quantity_available === 0 ? (
                              <h2 className="text-md font-bold text-red-700">SOLD</h2>
                            ) : (
                              <>
                                <h2 className="text-md">Quantity</h2>
                                <select className="p-2" onChange={(e) => updateTicketQuantity(index, parseInt(e.target.value))} value={ticketQuantities[index]}>
                ...
                "
                      >
                        <h1 className=" font-bold">Total ticket : {totalQuantity}</h1>
                        <h1 className=" font-bold">Total Price : Rp. {totalPrice}</h1>
                      </div>
