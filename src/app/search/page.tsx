  ...
  const dynamicQuery = (key: string, value: string, condition?: string) => {
    ...
    if (value && key !== 'pricemin' && key !== 'pricemax' && !condition) {
      ...
      if (valueUpdate) {
        ...
        const filteredValues = currentValues.filter((values: any) => values !== value);
                ...
                <AccordionItem value="category">
                  ...
                  <AccordionContent>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Music</label>
                      <input type="checkbox" id="category" name="Music" value="Music" checked={isChecked('cat', 'Music')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Festival</label>
                      <input type="checkbox" id="category" name="Festival" value="Festival" checked={isChecked('cat', 'Festival')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>International</label>
                      <input type="checkbox" id="category" name="International" value="International" checked={isChecked('cat', 'International')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Nature</label>
                      <input type="checkbox" id="category" name="Nature" value="Nature" checked={isChecked('cat', 'Nature')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Adventure</label>
                      <input type="checkbox" id="category" name="Adventure" value="Adventure" checked={isChecked('cat', 'Adventure')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Foodies</label>
                      <input type="checkbox" id="category" name="Foodies" value="Foodies" checked={isChecked('cat', 'Foodies')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Health</label>
                      <input type="checkbox" id="category" name="Health" value="Health" checked={isChecked('cat', 'Health')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Travel</label>
                      <input type="checkbox" id="category" name="Travel" value="Travel" checked={isChecked('cat', 'Travel')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                    <div className="flex gap-2 justify-between items-center">
                      <label>Pop Culture</label>
                      <input type="checkbox" id="category" name="PopCulture" value="PopCulture" checked={isChecked('cat', 'PopCulture')} onChange={(e) => handlingFilter('cat', e)} />
                    </div>
                  </AccordionContent>
                </AccordionItem>
                        ...
                        .map((value: any, index: number) => {
                          return (
                            <div className="flex justify-between" key={index}>
                              ...
                              <input type="checkbox" id="location" name={`${value.city_name}`} value={`${value.city_name}`} checked={isChecked('city', `${value.city_name}`)} onChange={(e) => handlingFilter('city', e)} />
                            </div>
                          );
                        })
                        .slice(0, 5)}
                ...
                <AccordionItem value="price">
                  ...
                  <AccordionContent className="flex flex-col gap-2">
                    <div className="flex gap-2 items-center w-fit">
                      <input type="number" id="price" name="pricemin" className="border rounded-sm p-2 shadow-sm" placeholder="Price from Rp..." value={pricemin} onChange={(e) => handlePriceFilterPriceMin(e)} /> -
                      <input type="number" id="price" name="pricemax" className="border rounded-sm p-2 shadow-sm" placeholder="Price to Rp..." value={pricemax} onChange={(e) => handlePriceFilterPriceMax(e)} />
                    </div>
                ...
                <AccordionItem value="date">
                  ...
                  <AccordionContent className="flex flex-col gap-2">
                    <div className="flex gap-2 items-center w-full">
                      <input type="date" id="price" name="startdate" className="border rounded-sm p-2  shadow-sm" placeholder="Price from Rp..." onChange={(e) => handlingDateFilterFrom(e)} />
                      <input type="date" id="price" name="enddate" className="border rounded-sm p-2  shadow-sm" placeholder="Price to Rp..." onChange={(e) => handlingDateFilterTo(e)} />
                    </div>
                ...
                filterData.map((value: any, index: number) => {
                  return (
                    <EventCard
                      ...
                      eventPrice={(filterData.length > 0 ? Math.min(...filterData[index].ticket_types.map((ticket: any) => ticket.price)) : 0) as any}
