  ...
  return (
    <>
      ...
      <div className="bg-white w-full h-full py-14 px-10 md:px-32 lg:px-48 lg:py-20 lg:pb-24 flex flex-col gap-14 lg:gap-20 ">
        ...
        {allEvent?.topEvents ? (
          <EventCarousel
            ...
            eventDataFromLandingPage={allEvent.categories}
          />
        ) : (
      ...
      <div className="bg-blue-950">
        <div className=" w-full h-full px-10 md:px-32 lg:px-48 py-16 lg:py-20 flex flex-col gap-20">
          {allEvent?.topEvents ? (
            <TopEvent eventData={allEvent.topEvents} />
          ) : (
            <TopEventsSkeleton />
          )}
        </div>
      ...
      <div className="bg-white w-full h-full px-10 md:px-32 py-8 lg:py-10 lg:px-48 flex flex-col gap-20">
        {allEvent?.categories ? (
          <Banner eventDataFromLandingPage={allEvent.categories} />
        ) : (
          <BannerSkeleton />
        )}
      </div>
      <div className="bg-white w-full h-full px-10 md:px-32 lg:px-48 py-8 lg:py-12 flex flex-col gap-20">
        {allEvent?.location ? (
          <CarouselRounded eventDataFromMain={allEvent.organizer} />
        ) : (
          <CarouselRoundedSkeleton />
        )}
      </div>
      <div className="bg-white w-full h-full px-10 md:px-32 lg:px-48 py-16 lg:py-20 lg:pb-28 flex flex-col gap-20">
        {allEvent?.topEvents ? (
          <EventCarousel
            ...
            eventDataFromLandingPage={allEvent.categoriesTwo}
          />
        ) : (
          <EventCarouselSkeleton />
        )}
